...
static const item_t MENU_DESKTOP[] = {
    ...
    { "  Restore all windows",CMID_DESKTOP_RESTORE_WINS,    0, 0 },
...
/* Dock menu kept for ABI/ID stability but dock is removed in v3. */
static const item_t MENU_DOCK[] __attribute__((unused)) = {
...
static void dispatch(ctx_menu_id_t id, const ctx_hit_t *hit)
{
    ...
    case CMID_DESKTOP_TERMINAL:
        ...
        break;
    case CMID_DESKTOP_RESTORE_WINS:
        desktop_restore_windows();
        break;
    ...
    case CMID_WIN_CLOSE:
        if (hit->win_handle >= 0) {
            ...
            comp_set_visible(hit->win_handle, 0);
            /* BUG FIX: same class of bug as input_router_task_exit() (see
             * kernel/drivers/ps2/input_router.c) — this hardcoded
             * input_router_set_focus(0) sent focus to task slot 0
             * (typically init_task, which never reads its input queue)
             * instead of the desktop task, permanently dropping all
             * subsequent keyboard input whenever a window was closed via
             * the right-click context menu. g_desktop_task is the
             * correctly tracked desktop task id. */
            input_router_set_focus(g_desktop_task);
        }
        break;
    ...
    case CMID_TB_MINIMIZE:
        if (hit->win_handle >= 0) {
            comp_set_visible(hit->win_handle, 0);
            /* BUG FIX: same class as CMID_WIN_CLOSE above. */
            input_router_set_focus(g_desktop_task);
        }
        break;
    ...
    /* ── Dock (removed — stubs kept for ABI compat) ───────── */
    case CMID_DOCK_OPEN_APP:
        if (hit->item_name[0]) appman_launch(hit->item_name);
        break;
    case CMID_DOCK_PIN_UNPIN:
    case CMID_DOCK_CLOSE_WINDOW:
    case CMID_DOCK_APP_SETTINGS:
        kinfo("CTX_MENU: dock action (dock removed)\n");
        break;
...
void ctx_menu_open(const ctx_hit_t *hit)
{
    ...
    case CTX_TARGET_TASKBAR_ITEM:
        items = MENU_WIN_BODY; count = MENU_WIN_BODY_COUNT; break;  /* fallback */
