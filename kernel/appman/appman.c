/* kernel/appman/appman.c — Application manager
 *
...
#include "appman.h"
#include "../../apps/widget_demo/widget_demo.h"
...
void appman_init(void) {
    /* GUI finalization pass: every other built-in app (Terminal, Text
     * Editor, File Manager, System Monitor, Calculator, Settings,
     * Package Manager, Draco Shield, Draco Manager, Login Manager, Paint,
     * Image Viewer, Media Player) was removed from the running OS — their
     * old hand-drawn implementations are gone, replaced with empty stub
     * folders under apps/<name>/ (e.g. apps/terminal/terminal.c) so each
     * can be properly rebuilt on top of the widget toolkit
     * (gui/widgets/widgets.h) and re-registered here when ready. See
     * docs/CHANGELOG.md for the full list and rationale. Widget Demo is
     * the only currently-registered app. */
    appman_register("Widget Demo", "Accessories", app_widget_demo);
