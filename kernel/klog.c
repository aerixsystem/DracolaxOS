...
/* Create a new log file under /storage/main/logs/ via VFS path. */
static void open_new_log_file(log_chan_t *c) {
    char name[64];
    make_log_name(c, c->file_idx, name, (int)sizeof(name));

    /* Use vfs_open to resolve /storage/main/logs and create file there */
    vfs_node_t *logs_dir = vfs_open("/storage/main/logs");
    if (!logs_dir && storage_root) {
        /* fallback: write to storage root so logs are never silently lost */
        logs_dir = storage_root;
    }
    if (!logs_dir) {
        kerror("KLOG: no logs directory available — log file '%s' lost\n", name);
        return;
    }

    ramfs_create(logs_dir, name);
    c->node      = vfs_finddir(logs_dir, name);
    c->write_off = 0;

    if (c->node)
        kinfo("KLOG: opened log file '%s'\n", name);
    else
