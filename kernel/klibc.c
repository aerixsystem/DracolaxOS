...
void utoa64(uint64_t val, char *buf, int base) {
    ...
    char tmp[68];
    /* BUG FIX (numbers print with garbage appended, e.g. "task 111"
     * logged for task id 11): tmp[67] is the null terminator, so the
     * first digit written must land at tmp[66] for the digit run to be
     * contiguous with it. This used to start i at 66, so the first write
     * went to tmp[--i] == tmp[65] — skipping tmp[66] entirely and leaving
     * it as an uninitialized stack byte between the real digits and the
     * terminator. strcpy() below copies from the start of the digit run
     * up to the first zero byte it finds — for any value needing 2+
     * digits, that gap byte sat right in the middle of the copied range,
     * appending whatever garbage happened to be on the stack (often a
     * leftover digit character from a previous call, since this
     * function's own tmp buffer tends to reuse the same stack slot
     * across calls) before eventually reaching the real terminator.
     * Starting i at 67 instead makes the first write land at tmp[66],
     * directly adjacent to the terminator with no gap. */
    int  i = 67;   /* fill from end — no reverse pass needed */
    tmp[67] = '\0';
