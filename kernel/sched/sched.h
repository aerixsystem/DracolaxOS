 ...
 * up): sched_watchdog_check() already detects tasks whose heartbeat
 * stalls, but previously that only ever resulted in kwarn() log lines
 * and, eventually, killing the task — nothing surfaced this to the user
 * visually. sched_is_system_slow() reports true once heartbeat strikes
 * have been happening frequently across recent watchdog checks (see
 * sched.c for the exact rolling-counter logic) — the desktop can poll
 * this once it manages to run and show a one-time banner, e.g. "this
 * system appears to be running slowly." Deliberately NOT tied to any
 * one specific task (a slow host causes stalls across many tasks, not
 * just the desktop) and deliberately a simple heuristic, not a precise
 * measurement — the goal is "warn the user something's off," not
