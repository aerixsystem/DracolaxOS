...
void sched_exit(void) {
    ...
    /* Capture the stack pointer to free — but we CANNOT call kfree() yet because
     * kfree() poisons the payload with 0xCC, which includes the bytes we are still
     * executing from.  Instead we stash the pointer in a global "pending free" slot
     * and let the scheduler release it on the NEXT tick, once we are safely running
     * on a different stack. */
    void *dying_stack = tasks[cur].stack;
    tasks[cur].stack = NULL;

    /* Flush this task's input queue and reassign focus */
    ...
    if (next == cur) {
        ...
        /* Safe to free here — we are going into hlt loop, not returning */
        if (dying_stack) kfree(dying_stack);
        for (;;) __asm__ volatile ("hlt");
    }

    /* Store dying stack for deferred free — the next timer tick in sched_tick
     * will release it after we have fully transitioned to `next`'s stack. */
    static void *s_pending_stack_free = NULL;
    if (s_pending_stack_free) {
        /* Free whatever was left from a prior exit (safe — we are on `cur`'s
         * stack which is different from s_pending_stack_free). */
        kfree(s_pending_stack_free);
    }
    s_pending_stack_free = dying_stack;

    tasks[next].state = TASK_RUNNING;
    cur = next;

    /* Switch to next task's stack and resume it — safe because we are now
     * running on next's stack, not on the poisoned dying stack. */
    __asm__ volatile (
        "mov %0, %%rsp\n"
        "pop %%r15\n"
        "pop %%r14\n"
...
int sched_kill(int id) {
    ...
    /* If killing another task (not current), free its stack immediately — safe
     * because we are running on a different stack.
     * If killing OURSELVES, defer the free (same poison-on-current-stack bug
     * as sched_exit) — sched_exit() handles the deferred free. */
    if (id != cur) {
    ...
    if (id == cur) sched_exit();   /* sched_exit defers the stack free */
    return 0;
}

/* BUG FIX (add a warning when the host is too slow for the OS to keep
 * up): rolling counter of how many strikes have been issued (across ALL
 * tasks) in recent sched_watchdog_check() calls. Halved each time the
 * function runs with zero fresh strikes, so a single rough patch (e.g.
 * one slow disk op) doesn't linger and trip the threshold long after
 * things are fine again — only SUSTAINED stalling keeps it elevated. */
static int g_recent_strikes = 0;
#define SLOW_SYSTEM_STRIKE_THRESHOLD 6

int sched_is_system_slow(void) {
    return g_recent_strikes >= SLOW_SYSTEM_STRIKE_THRESHOLD;
}
...
void sched_watchdog_check(void) {
    ...
    if (strikes_this_round > 0) g_recent_strikes += strikes_this_round;
    else if (g_recent_strikes > 0) g_recent_strikes /= 2;
}
