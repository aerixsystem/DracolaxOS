...
static void mount_storage(void) {
    ...
    if (sys_vol) {
        ...
        rwrite(sys_vol, "motd.txt",
               ...
               "  ls /storage  browse persistent storage\n"
...
void init_task(void) {
    ...
    if (g_boot_mode == BOOT_MODE_SHELL) {
        kinfo("INIT: [SHELL] minimal boot — no atlas, no security, no desktop\n");
        mount_ramfs();
        kinfo("INIT: [SHELL] launching shell\n");
        sched_sleep(150);
        shell_run();
    ...
    if (bootmode_wants_desktop() && fb.available) {
        ...
        kinfo("INIT: text mode — launching shell\n");
        sched_sleep(200);
        shell_run();
    }
