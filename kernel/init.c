...
#include "../gui/icons/icon_data.h"   /* embedded .dxi icon data */
#include "../services/notification_daemon.h"
...
static void rwrite(vfs_node_t *root, const char *name, const char *text) {
    ...
    if (n) vfs_write(n, 0, (uint32_t)strlen(text), (const uint8_t *)text);
                ...
                "(heap exhausted or RAMFS_MAX_FILES=%d exceeded)\n",
...
static void mount_storage(void) {
    ...
    /* Write stub entries so 'ls /storage' lists the subdirectories.
     * (Sub-volumes are separate mounts that overlay these paths but the
     * listing comes from storage_root itself.) */
    rmkdir(storage_root, "main");
    rmkdir(storage_root, "usb");
    rmkdir(storage_root, "network");
    rmkdir(storage_root, "ramdisk");
    rmkdir(storage_root, "apps");
    ...
    if (sys_vol) {
        ...
        rwrite(sys_vol, "version.txt",
               ...
               "Author  : Lunax (Yunis) and Amilcar\n"
               "Build   : 2026-03-31\n");
        ...
        rwrite(sys_vol, "motd.txt",
               ...
               "  ls /storage/main         browse internal storage\n"
               "  ls /storage/main/logs    view system logs\n"
               "  klog                     read latest kernel log\n"
               "  login root               admin login (password: dracolax)\n"
               "  draco install <pkg>      install a package\n"
               "  exec <elf>               run ELF64 binary\n"
               "\n"
               "Storage layout (draco://storage/...):\n"
               "  /storage/main/system     OS core files (id2=001)\n"
               "  /storage/main/users      User accounts  (id2=002)\n"
               "  /storage/main/apps       Installed apps (id2=003)\n"
               "  /storage/main/temp       Sys temp, auto-cleaned (id2=004)\n"
               "  /storage/main/cache      Thumbnails, updates (id2=005)\n"
               "  /storage/main/logs       System logs (id2=006)\n"
               "  /storage/ramdisk         Volatile fast storage (node=04)\n"
               "  /storage/apps            Built-in global apps (node=05)\n");
        rwrite(sys_vol, "fstab.txt",
               "# DracolaxOS fstab — draco://storage/...\n"
               "/ramfs                          ramfs  volatile     0  0\n"
               "/proc                           procfs kernel       0  0\n"
               "/storage                        ramfs  node=01      0  0\n"
               "/storage/main                   ramfs  node=01      0  0\n"
               "/storage/main/system            ramfs  id2=001      0  0\n"
               "/storage/main/system/shared     ramfs  id2=001a     0  0\n"
               "/storage/main/users             ramfs  id2=002      0  0\n"
               "/storage/main/apps              ramfs  id2=003      0  0\n"
               "/storage/main/temp              ramfs  id2=004,auto-clean  0  0\n"
               "/storage/main/cache             ramfs  id2=005      0  0\n"
               "/storage/main/logs              ramfs  id2=006      0  0\n"
               "/storage/usb                    ramfs  node=02      0  0\n"
               "/storage/network                ramfs  node=03      0  0\n"
               "/storage/ramdisk                ramfs  node=04,volatile  0  0\n"
               "/storage/apps                   ramfs  node=05      0  0\n");
    ...
    if (icons_vol) {
        ...
        /* ── Populate built-in .dxi icons from embedded data ────────────
         * Each icon is already compiled into the kernel image via icon_data.h.
         * We create a VFS file for each and write the raw .dxi bytes so that
         * dxi_load() can find them at runtime via the normal path:
         *   /storage/main/system/shared/images/<name>.dxi
         * ────────────────────────────────────────────────────────────── */
        for (int bi = 0; bi < BUILTIN_ICON_COUNT; bi++) {
            const builtin_icon_t *ic = &builtin_icons[bi];
            ramfs_create(icons_vol, ic->dxi_name);
            vfs_node_t *f = vfs_finddir(icons_vol, ic->dxi_name);
            if (f) {
                vfs_write(f, 0, ic->size, ic->data);
                kinfo("INIT: icon %s loaded (%u bytes)\n", ic->dxi_name, (unsigned)ic->size);
            } else {
                kwarn("INIT: icon %s — vfs_finddir failed\n", ic->dxi_name);
            }
        }
    } else {
    ...
    if (users_vol) {
        ...
        /* Create default user directories per storage spec:
         *   /storage/main/users/<username>/documents  (tags: docs, project)
         *   /storage/main/users/<username>/downloads  (tags: downloads)
         *   /storage/main/users/<username>/desktop    (tags: ui, workspace)
         *   /storage/main/users/<username>/configs    (tags: settings)
         *   /storage/main/users/<username>/pictures
         *   /storage/main/users/<username>/music
         *   /storage/main/users/<username>/videos
         *   /storage/main/users/<username>/trash
         *   /storage/main/users/<username>/temp       (hidden, auto-cleared)
         */
        static const char *usernames[] = { "root", "guest", NULL };
        static const char *home_dirs[] = {
            "documents", "downloads", "desktop", "configs",
            "pictures",  "music",     "videos",  "trash",
            "temp", NULL
        };
        for (int ui = 0; usernames[ui]; ui++) {
            rmkdir(users_vol, usernames[ui]);
            /* Per-user metadata */
            char umeta[128];
            snprintf(umeta, sizeof(umeta),
                "{\"username\":\"%s\",\"node_id\":\"00201\","
                "\"created\":\"2026-01-01\"}\n", usernames[ui]);
            (void)umeta;

            /* BUG FIX (cd into home subdirs): /storage/main/users is a flat
             * RAMFS volume — "guest"/"root" are flat name entries with no
             * real children (no finddir/readdir). cmd_cd now validates its
             * target exists (see kernel/shell.c flat_dir_exists), so
             * `cd documents` from /storage/main/users/guest would fail
             * unless something marks that virtual directory as present.
             *
             * Create each home subdir as a flat DIR-typed marker entry on
             * storage_root, using the same "main/users/<user>/<dir>" flat
             * naming convention that cmd_create/cmd_ls already use for
             * files inside these virtual directories. This makes
             * `cd documents` (and `ls` showing it under the user's home)
             * work without needing a real hierarchical filesystem. */
            for (int hi = 0; home_dirs[hi]; hi++) {
                char flat_name[96];
                snprintf(flat_name, sizeof(flat_name), "main/users/%s/%s",
                         usernames[ui], home_dirs[hi]);
                rmkdir(storage_root, flat_name);
            }
        }
        kinfo("INIT: /storage/main/users mounted (root guest)\n");
    }
    ...
    if (apps2_vol) {
        ...
        /* Populate app registry as flat files in a single RAMFS volume.
         * RAMFS is flat (no real subdirectories), so we write one file per
         * app named "<appslug>.json".  The file contains JSON with name,
         * category and icon path so the file manager / package manager can
         * enumerate apps without needing a real filesystem hierarchy. */
        static const struct {
            const char *slug; const char *name; const char *cat; const char *icon;
        } app_meta[] = {
            {"terminal",        "Terminal",        "System",      "terminal.dxi"},
            {"file-manager",    "File Manager",    "System",      "file-manager.dxi"},
            {"text-editor",     "Text Editor",     "Accessories", "text-editor.dxi"},
            {"calculator",      "Calculator",      "Accessories", "calculator.dxi"},
            {"settings",        "Settings",        "System",      "settings.dxi"},
            {"system-monitor",  "System Monitor",  "System",      "system-monitor.dxi"},
            {"package-manager", "Package Manager", "System",      "package-manager.dxi"},
            {"draco-shield",    "Draco Shield",    "System",      "draco-shield.dxi"},
            {"draco-manager",   "Draco Manager",   "System",      "draco-manager.dxi"},
            {"login-manager",   "Login Manager",   "System",      "login-manager.dxi"},
            {"paint",           "Paint",           "Graphics",    "paint.dxi"},
            {"image-viewer",    "Image Viewer",    "Graphics",    "image-viewer.dxi"},
            {"media-player",    "Media Player",    "Multimedia",  "media-player.dxi"},
            {"disk-manager",    "Disk Manager",    "System",      "disk-manager.dxi"},
            {"trash-manager",   "Trash Manager",   "System",      "trash-manager.dxi"},
        };
        for (size_t ai = 0; ai < sizeof(app_meta)/sizeof(app_meta[0]); ai++) {
            char fname[72], meta[256];
            snprintf(fname, sizeof(fname), "%s.json", app_meta[ai].slug);
            snprintf(meta, sizeof(meta),
                "{\"name\":\"%s\",\"category\":\"%s\","
                "\"icon\":\"/storage/main/system/shared/images/%s\","
                "\"flags\":\"builtin\"}\n",
                app_meta[ai].name, app_meta[ai].cat, app_meta[ai].icon);
            rwrite(apps2_vol, fname, meta);
            kinfo("INIT: app entry %s written\n", fname);
        }
    }
    ...
    if (logs_vol) {
        ...
        /* Seed a boot marker so klog can write immediately */
        rwrite(logs_vol, "boot.log", "[BOOT] DracolaxOS init started\n");
        kinfo("INIT: /storage/main/logs mounted\n");
    }

    /* ── Node 01e: /storage/main/temp — system-wide temp (auto-cleared on shutdown) */
    vfs_node_t *temp_vol = ramfs_new("storage-temp");
    if (temp_vol) {
        vfs_mount("/storage/main/temp", temp_vol);
        rwrite(temp_vol, ".autoclean",
               "This directory is cleared on shutdown/reboot.\n");
        kinfo("INIT: /storage/main/temp mounted\n");
    }

    /* ── Node 01f: /storage/main/cache — thumbnails, update cache, misc */
    vfs_node_t *cache_vol = ramfs_new("storage-cache");
    if (cache_vol) {
        vfs_mount("/storage/main/cache", cache_vol);
        rmkdir(cache_vol, "thumbnails");
        rmkdir(cache_vol, "updates");
        rmkdir(cache_vol, "packages");
        kinfo("INIT: /storage/main/cache mounted (thumbnails updates packages)\n");
    }
    ...
    /* ── Node 05: /storage/apps — global built-in apps (Node ID: 05) ── */
    vfs_node_t *apps_vol = ramfs_new("storage-apps");
    if (apps_vol) {
        vfs_mount("/storage/apps", apps_vol);
        /* Per-app subdirs: data / configs / cache
         * RAMFS is flat so we write sentinel files named <app>_<subdir>.marker
         * The file manager and package manager read these to enumerate apps. */
        static const char *app_names[] = {
            "terminal", "file-manager", "text-editor", "calculator",
            "settings", "system-monitor", "package-manager",
            "draco-shield", "draco-manager", "login-manager",
            "paint", "image-viewer", "media-player",
            "disk-manager", "trash-manager", NULL
        };
        for (int ai = 0; app_names[ai]; ai++) {
            char buf[96];
            snprintf(buf, sizeof(buf), "%s.app", app_names[ai]);
            char meta[256];
            snprintf(meta, sizeof(meta),
                "{\"app\":\"%s\",\"node_id\":\"05\","
                "\"dirs\":[\"data\",\"configs\",\"cache\"]}\n",
                app_names[ai]);
            rwrite(apps_vol, buf, meta);
        }
        kinfo("INIT: /storage/apps mounted (%d built-in apps)\n",
              (int)(sizeof(app_names)/sizeof(app_names[0]) - 1));
    }
...
#define IRQ12_MAX_RETRIES 5
    ...
    /* BUG FIX (IRQ12 stall spam / Case C): previously, "both IRQs quiet"
     * fired the full drain+keyboard_reinit() recovery on *every* transition
     * into idle (quiet_rounds == 1) — including completely normal user
     * idle periods (stopped typing/moving the mouse for >=5s). That caused
     * a spurious keyboard_reinit() + PIC remask cycle every time the user
     * simply paused, spamming the log and briefly disturbing input state.
     *
     * Recovery should only run for a genuine "VM lost and regained focus"
     * event, which looks like: both IRQs were ACTIVE recently (the machine
     * was in use), then both go quiet for a SUSTAINED period (not just one
     * 5s tick — a normal pause), and recovery runs ONCE per quiet streak
     * (not re-armed until activity resumes). A machine that has never seen
     * any input (e.g. no PS/2 mouse, user hasn't typed yet) won't trigger
     * this at all — that's already handled by Case B's bounded retries. */
    int had_recent_activity   = 0;  /* both IRQs have fired at least once */
    int case_c_done_this_quiet = 0; /* recovery already ran for this idle streak */
#define CASE_C_QUIET_THRESHOLD 6     /* ~30s of total silence before recovering */
    ...
    for (;;) {
        ...
        if (d1 || d12) {
            /* Activity this round: both having ever fired marks the machine
             * as "in active use", and resets the quiet-streak recovery latch
             * so a future genuine stall can trigger recovery again. */
            had_recent_activity = 1;
            quiet_rounds = 0;
            case_c_done_this_quiet = 0;
        } else {
            quiet_rounds++;
        }
         ...
         *
         * Only run this when: the machine has seen real activity before
         * (had_recent_activity), the silence has lasted long enough to be
         * suspicious (quiet_rounds >= CASE_C_QUIET_THRESHOLD, ~30s — a
         * normal "user stepped away" pause won't trip this), and we
         * haven't already recovered during this quiet streak
         * (case_c_done_this_quiet). The latch only resets on the next
         * burst of real activity. */
        if (had_recent_activity &&
            quiet_rounds == CASE_C_QUIET_THRESHOLD &&
            !case_c_done_this_quiet) {
            case_c_done_this_quiet = 1;
            kwarn("IRQ-WDG: both IRQs quiet for %us — draining PS/2 buffer "
                  "and running keyboard_reinit()\n",
                  (unsigned)(CASE_C_QUIET_THRESHOLD * 5));
            ...
            /* BUG FIX (self-sustaining IRQ12 recovery loop): prev1/prev12
             * below are normally set from cur1/cur12 as snapshotted at
             * the TOP of this same iteration — i.e. BEFORE the recovery
             * actions above ran. If keyboard_reinit() or the aux-port
             * reassert itself causes the PS/2 controller to fire a
             * spurious IRQ1/IRQ12 (plausible — re-enabling scanning/aux
             * reporting is exactly the kind of transition that can
             * trigger one), that spurious tick shows up as a fresh
             * "d1||d12 != 0" on the VERY NEXT round, since the stale
             * pre-recovery baseline hasn't absorbed it yet. That reset
             * had_recent_activity/quiet_rounds/case_c_done_this_quiet,
             * arming Case C to fire again after another ~30s — forever,
             * with no real user input involved at all. Observed in
             * practice as "IRQ-WDG: both IRQs quiet for 30s" repeating
             * indefinitely on an otherwise-idle desktop. Beyond the log
             * noise, repeatedly draining/reinitializing the PS/2
             * controller during normal use risks eating a real keystroke
             * or mouse packet that happens to arrive during the drain
             * window — a plausible contributor to "clicks/keys
             * intermittently don't register."
             *
             * Fix: give any recovery-induced IRQ a moment to land, then
             * re-snapshot cur1/cur12 to include it, so it's absorbed into
             * this round's baseline instead of being misread as fresh
             * activity on the next one. */
            sched_sleep(50);
            cur1  = g_irq1_count;
            cur12 = g_irq12_count;
        }
        ...
        /* BUG FIX (add a warning when the host is too slow for the OS to
         * keep up): sched_is_system_slow() reports true once heartbeat
         * strikes have been happening frequently — a slow/overloaded
         * host (e.g. VirtualBox without hardware acceleration, or a
         * loaded host machine) causes exactly this pattern, and
         * previously it only ever showed up as kwarn() log lines a user
         * wouldn't normally see. Pushed through the notification
         * daemon's own toast system (services/notification_daemon.c),
         * which draws directly to the framebuffer from its own
         * independently-scheduled task — so this warning can still
         * reach the screen even if desktop_task itself is one of the
         * tasks currently struggling to keep up. Fired once per
         * transition into the slow state (not every watchdog cycle
         * while it stays true), so it doesn't spam. */
        {
            static int was_slow = 0;
            int is_slow = sched_is_system_slow();
            if (is_slow && !was_slow) {
                notif_push("System running slowly",
                          "DracolaxOS is detecting delayed task scheduling. "
                          "If running in a VM, try enabling hardware "
                          "virtualization/acceleration, or closing other "
                          "programs on the host.", 1);
            }
            was_slow = is_slow;
        }
