...
/* Expose desktop task ID so keyboard IRQ can route Super+letter directly */
extern volatile int g_desktop_task;

/* Push a character into a specific task's queue (not just the focused task).
 * Safe to call from task context. NOT safe from IRQ context. */
void input_router_push_to(int task_id, char c);

/* Register the desktop task so Super+letter shortcuts always reach it. */
