...
/** Return bitmask of buttons that transitioned 0->1 during last vmmouse_poll,
 *  then clear the latch.  Use instead of mouse_btn_pressed() when vmmouse is
 *  active so that brief clicks that arrive and release within one drain aren't
 *  lost.  When vmmouse is NOT active, returns 0 (use mouse_btn_pressed()). */
uint8_t vmmouse_get_pressed(void);
uint8_t vmmouse_get_released(void);
