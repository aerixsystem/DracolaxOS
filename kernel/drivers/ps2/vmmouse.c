...
/* Pressed-event latch: bits set when any packet in this drain saw a 0→1
 * button transition.  Consumed by vmmouse_get_pressed() and reset each poll. */
static uint8_t vm_pressed_latch = 0;
static uint8_t vm_released_latch = 0;

uint8_t vmmouse_get_pressed(void)  { uint8_t v=vm_pressed_latch;  vm_pressed_latch=0;  return v; }
uint8_t vmmouse_get_released(void) { uint8_t v=vm_released_latch; vm_released_latch=0; return v; }
...
void vmmouse_poll(void) {
    ...
    uint8_t prev_btns = (uint8_t)mouse_get_buttons();
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        /* Track press/release transitions within the drain */
        vm_pressed_latch  |= (uint8_t)( btns & ~prev_btns);  /* 0→1 bits */
        vm_released_latch |= (uint8_t)(~btns &  prev_btns);  /* 1→0 bits */
        prev_btns = btns;
    }

    if (sx_last >= 0) {
        mouse_set_pos(sx_last, sy_last);
        mouse_set_buttons(prev_btns);  /* last known button state */
    }
}
