...
CFLAGS := -ffreestanding -std=c11 $(OPT_FLAGS) -Wall -Wextra \
          ...
          -Iapps/text_editor \
          -Iapps/file_manager \
          -Iapps/system_monitor \
          -Iapps/calculator \
          -Iapps/settings \
          -Iapps/package_manager \
          -Iapps/draco_shield \
          -Iapps/draco_manager \
          -Iapps/login_manager \
          -Iapps/paint \
...
# kernel-linked built-in apps — one folder per app under apps/, each with
# its own <name>.c + <name>.h (installer is boot-time infrastructure, not
# an appman-registered launchable app, but follows the same convention).
# All apps except widget_demo are currently empty stubs — see
# docs/CHANGELOG.md for the GUI finalization pass that introduced this.
APPS := \
    apps/installer/installer.c \
    apps/widget_demo/widget_demo.c \
    apps/terminal/terminal.c \
    apps/text_editor/text_editor.c \
    apps/file_manager/file_manager.c \
    apps/system_monitor/system_monitor.c \
    apps/calculator/calculator.c \
    apps/settings/settings.c \
    apps/package_manager/package_manager.c \
    apps/draco_shield/draco_shield.c \
    apps/draco_manager/draco_manager.c \
    apps/login_manager/login_manager.c \
    apps/paint/paint.c \
    apps/image_viewer/image_viewer.c \
    apps/media_player/media_player.c
