/* apps/installer/installer.c — DracolaxOS First-Boot Installer v2.0
 *
 * Features:
 ...
 *
 * BUG FIX (watchdog): these polling loops run with interrupts enabled but
 * never call sched_yield(). On slow/virtualized IDE controllers each loop
 * can burn its full iteration budget (100000 / 300000 spins), and
 * detect_other_os() calls this multiple times back-to-back during
 * installer_run() / desktop_task startup. Without a yield, init_task's or
 * desktop_task's heartbeat doesn't advance for long enough that the
 * irq_watchdog_task (sched_watchdog_check, every ~5s) can register up to
 * WATCHDOG_STRIKE_MAX misses and kill the task mid-boot.
 * Yielding every N spins keeps the heartbeat fresh without materially
 * slowing down the (already best-effort, fail-safe) detection. */
static int pio_read_sector(uint32_t lba, uint8_t *buf512) {
    /* Wait for drive ready */
    int t = 100000;
    while (t-- && (inb_det(0x1F7) & 0xC0) != 0x40) {
        if ((t & 0xFFF) == 0) sched_yield();
    }
